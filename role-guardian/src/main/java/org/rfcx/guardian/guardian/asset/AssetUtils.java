...
public class AssetUtils {
    ...
    public void runFileSystemAssetCleanup(int checkFilesUnModifiedSinceThisManyMinutes) {
        ...
        // CheckIn Databases
        for (String[] row : app.apiCheckInDb.dbQueued.getAllRows()) {
            assetFilePathsFromDatabase.add(row[4]);
        }
        for (String[] row : app.apiCheckInDb.dbStashed.getAllRows()) {
            assetFilePathsFromDatabase.add(row[4]);
        }
        for (String[] row : app.apiCheckInDb.dbSent.getAllRows()) {
            assetFilePathsFromDatabase.add(row[4]);
        }
        for (String[] row : app.apiCheckInDb.dbSkipped.getAllRows()) {
            assetFilePathsFromDatabase.add(row[4]);
        }
        // Audio Encode Databases
        for (String[] row : app.audioEncodeDb.dbEncoded.getAllRows()) {
            assetFilePathsFromDatabase.add(row[10]);
        }
        for (String[] row : app.audioClassifyDb.dbQueued.getAllRows()) {
            assetFilePathsFromDatabase.add(row[6]);
        }
        //for (String[] row : app.audioSnippetDb.dbQueued.getAllRows()) { assetFilePathsFromDatabase.add(row[6]); }
        // Asset Library Databases
        for (String[] row : app.assetLibraryDb.dbAudio.getAllRows()) {
            assetFilePathsFromDatabase.add(row[5]);
        }
        for (String[] row : app.assetLibraryDb.dbClassifier.getAllRows()) {
            assetFilePathsFromDatabase.add(row[5]);
        }
        // Classifier Databases
        for (String[] row : app.audioClassifierDb.dbActive.getAllRows()) {
            assetFilePathsFromDatabase.add(row[6]);
        }
