...
public class ApiCheckInHealthUtils {
    ...
    private final Map<String, String[]> inFlightCheckInEntries = new HashMap<>();
    private final Map<String, long[]> inFlightCheckInStats = new HashMap<>();
    ...
    public void setInFlightCheckInStats(String keyId, long msgSendStart, long msgSendDuration, long msgPayloadSize) {
        ...
        if (stats == null) {
            stats = new long[]{0, 0, 0};
        }
        if (msgSendStart != 0) {
            stats[0] = msgSendStart;
        }
        if (msgSendDuration != 0) {
            stats[1] = msgSendDuration;
        }
        if (msgPayloadSize != 0) {
            stats[2] = msgPayloadSize;
        }
    ...
    private void setOrResetRecentCheckInHealthCheck(long prefsAudioCycleDuration) {
        ...
        if (!healthCheckMonitors.containsKey(healthCheckCategories[0])
            ...
            for (String healthCheckCategory : healthCheckCategories) {
                if (!healthCheckMonitors.containsKey(healthCheckCategory)) {
                    healthCheckMonitors.put(healthCheckCategory, healthCheckInitValues);
                }
            }
    ...
    public boolean validateRecentCheckInHealthCheck(long prefsAudioCycleDuration, long[] currentCheckInStats) {
        ...
        long[] currAvgVals = new long[healthCheckCategories.length];
        Arrays.fill(currAvgVals, 0);
        ...
        for (int j = 0; j < healthCheckCategories.length; j++) {
            ...
            for (int i = (healthCheckMeasurementCount - 1); i > 0; i--) {
                arraySnapshot[i] = healthCheckMonitors.get(categ)[i - 1];
            }
    ...
    public boolean isBatteryChargeSufficientForCheckIn() {
        ...
        if (isBatteryChargeSufficient && (batteryChargeCutoff == 100)) {
            ...
            if (!isBatteryChargeSufficient) {
                Log.d(logTag, "Battery is at 100% but is not yet fully charged.");
            }
        }
