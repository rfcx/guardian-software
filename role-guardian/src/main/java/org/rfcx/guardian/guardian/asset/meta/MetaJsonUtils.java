...
public class MetaJsonUtils {
    ...
    public void createSystemMetaDataJsonSnapshot() throws JSONException {
        ...
        if (sentinelPower.length() > 0) {
            metaDataJsonObj.put("sentinel_power", sentinelPower);
        }
        ...
        String swmDiagnostic = getConcatMetaField(RfcxComm.getQuery("admin", "database_get_all_rows",
                "swm_meta", app.getResolver()));
        if (swmDiagnostic.length() > 0) {
            metaDataJsonObj.put("swm_diagnostic", swmDiagnostic);
        }
        ...
        if (app.deviceSystemDb.dbDateTimeOffsets.getCount() > 0) {
            dateTimeOffsets.add(app.deviceSystemDb.dbDateTimeOffsets.getConcatRows());
        }
        if (dateTimeOffsets.size() > 0) {
            metaDataJsonObj.put("datetime_offsets", TextUtils.join("|", dateTimeOffsets));
        }
    ...
    private void clearPrePackageMetaData(Date deleteBefore) {
        try {
            ...
            RfcxComm.deleteQuery("admin", "database_delete_rows_before", "swm_meta|" + deleteBefore.getTime(), app.getResolver());
...
//		long metaQueueFullByteLength = app.metaDb.dbMeta.getCumulativeJsonBlobLengthForAllRows();
        ...
        if (metaQueueFullByteLength >= metaFileSizeLimitInBytes) {
            ...
            for (String[] metaRowsToArchive : app.metaDb.dbMeta.getRowsWithOffset(metaQueueFullRecordCount - metaArchiveRecordCount, metaArchiveRecordCount)) {
                ...
                if (metaArchiveTimestamp == 0) {
                    metaArchiveTimestamp = Long.parseLong(metaRowsToArchive[1]);
                }
    ...
    public JSONObject retrieveAndBundleMetaJson(JSONObject inputMetaJson, int maxMetaRowsToBundle, boolean overrideFilterByLastAccessedAt) throws JSONException {
        ...
        for (String[] metaRow : metaRows) {
            ...
            if (metaJsonBundledSnapshotsObj == null) {
                ...
                while (bundleKeys.hasNext()) {
                    String bndlKey = bundleKeys.next();
                    if (!ArrayUtils.doesStringListContainString(allKeys, bndlKey)) {
                        allKeys.add(bndlKey);
                    }
                }
                while (appendKeys.hasNext()) {
                    String apnKey = appendKeys.next();
                    if (!ArrayUtils.doesStringListContainString(allKeys, apnKey)) {
                        allKeys.add(apnKey);
                    }
                }
            ...
            if (metaJsonBundledSnapshotsIds.length() >= maxMetaRowsToBundle) {
                break;
            }
        }
        ...
        if (metaJsonBundledSnapshotsObj == null) {
            metaJsonBundledSnapshotsObj = new JSONObject();
        }
    ...
    public String getCheckInStatusInfoForJson(String[] includeAssetIdLists) {
        ...
        for (String type : types) {
            ...
            if (type.equalsIgnoreCase("sent")) {
                ...
                if (includeIdList) {
                    idRows = app.apiCheckInDb.dbSent.getLatestRowsWithLimit(idBundleLimit);
                }
                ...
                if (includeIdList) {
                    idRows = app.apiCheckInDb.dbQueued.getLatestRowsWithLimit(idBundleLimit);
                }
                ...
                if (includeIdList) {
                    idRows = app.metaDb.dbMeta.getLatestRowsWithLimit(idBundleLimit);
                }
                ...
                if (includeIdList) {
                    idRows = app.apiCheckInDb.dbSkipped.getLatestRowsWithLimit(idBundleLimit);
                }
                ...
                if (includeIdList) {
                    idRows = app.apiCheckInDb.dbStashed.getLatestRowsWithLimit(idBundleLimit);
                }
