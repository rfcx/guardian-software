...
public class SentryAccelUtils {
    ...
    public boolean isChipAccessibleByI2c() {
        ...
        if (isNotExplicitlyDisabled) {
            isI2cHandlerAccessible = app.sensorI2cUtils.isI2cHandlerAccessible();
            if (isI2cHandlerAccessible) {
                String i2cConnectAttempt = app.sensorI2cUtils.i2cGetAsString("0x00", i2cMainAddr, true, true);
                isI2cAccelChipConnected = ((i2cConnectAttempt != null) && (Math.abs(SensorI2cUtils.twosComplementHexToDecAsLong(i2cConnectAttempt)) > 0));
    ...
    public void updateSentryAccelValues() {
        try {
            ...
            for (String[] i2cLabeledOutput : app.sensorI2cUtils.i2cGet(buildI2cQueryList(), i2cMainAddr, true, true)) {
