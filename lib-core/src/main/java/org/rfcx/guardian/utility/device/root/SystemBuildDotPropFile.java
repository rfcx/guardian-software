...
public class SystemBuildDotPropFile {
    ...
    public static void updateBuildDotPropFile(String[] propertiesWithValues, Context context, boolean rebootWhenFinished) {
        ...
        File shellScriptFileObj = new File(shellScriptFilePath);
        if (shellScriptFileObj.exists()) {
            shellScriptFileObj.delete();
        }
        ...
        if (rebootWhenFinished) {
            shellScriptContents.append("echo 'Triggering Reboot now...';\n").append("reboot;\n");
        }
