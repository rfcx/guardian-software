...
public class DeviceHardware_OrangePi_3G_IOT {
    ...
    public static final Map<String, String[]> DEVICE_GPIO_MAP = Collections.unmodifiableMap(new HashMap<String, String[]>() {{
        put("sentry_power", new String[]{"26", "write"});
...
//		put("sampling_rate_min",			new String[] { "hotplug", "30000" });
    ...
    public static boolean isDevice_OrangePi_3G_IOT() {
        for (String manufacturer : DEVICE_MANUFACTURER) {
            if (DeviceHardwareUtils.getManufacturer().equalsIgnoreCase(manufacturer)) {
                for (String model : DEVICE_MODEL) {
                    if (DeviceHardwareUtils.getModel().equalsIgnoreCase(model)) {
                        return true;
                    }
                }
            }
