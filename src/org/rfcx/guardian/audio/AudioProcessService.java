...
public class AudioProcessService extends Service {
	...
	private static final int PROCESSING_CHECK_DELAY = 6000;
			...
			try {
				while (audioProcessService.runFlag) {
					...
					for (File wavFile : wavFiles) {
						if (!wavFile.isDirectory() && ((currentTime - wavFile.lastModified()) > PROCESSING_CHECK_DELAY)) {
							...
							ffe.adjustAudioConfig(app.audioCore.CAPTURE_SAMPLE_RATE_HZ, 16, 1);
