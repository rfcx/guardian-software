...
for app in "${apps[@]}"; do
    ...
    curl -O https://s3-eu-west-1.amazonaws.com/rfcx-install/rfcx-guardian/guardian-android-guardian/production/$app-$version.apk

    echo "transferring apk to device...";
    $ADB_BIN push $SCRIPT_DIR/guardian-$version.apk data/local/tmp/$app-$version.apk;

    echo "performing installation...";
    $ADB_BIN shell pm install -f -r data/local/tmp/$app-$version.apk;

    echo "deleting apk...";
    $ADB_BIN shell rm data/local/tmp/$app-$version.apk;
